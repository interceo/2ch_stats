...
def analyze_thread_activity(df):
    """Анализ активности тредов"""
    print("Анализ активности тредов...")

    # 1. Распределение по количеству постов
    plt.figure(figsize=(14, 6))
    sns.histplot(df['posts_count'], bins=30, kde=True, color='skyblue')
    plt.title('Распределение тредов по количеству постов', fontsize=16)
    plt.xlabel('Количество постов', fontsize=14)
    plt.ylabel('Количество тредов', fontsize=14)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('stats/posts_distribution.png', dpi=300)
    plt.close()

    # 2. Распределение по категориям постов
    plt.figure(figsize=(12, 6))
    posts_category_counts = df['posts_category'].value_counts()
    sns.barplot(x=posts_category_counts.index, y=posts_category_counts.values, palette='viridis')
    plt.title('Распределение тредов по категориям активности', fontsize=16)
    plt.xlabel('Категория', fontsize=14)
    plt.ylabel('Количество тредов', fontsize=14)
    plt.xticks(rotation=45)
    plt.grid(True, axis='y')
    plt.tight_layout()
    plt.savefig('stats/posts_categories.png', dpi=300)
    plt.close()

    # 3. Соотношение уникальных пользователей к постам
    plt.figure(figsize=(14, 6))
    sns.scatterplot(x='posts_count', y='unique_posters', hue='op_activity',
                    size='total_files', sizes=(20, 200), alpha=0.7, data=df)
    plt.title('Соотношение количества постов и уникальных пользователей', fontsize=16)
    plt.xlabel('Количество постов', fontsize=14)
    plt.ylabel('Уникальные пользователи', fontsize=14)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('stats/posts_vs_users.png', dpi=300)
    plt.close()

def analyze_op_activity(df):
    """Анализ активности ОП"""
    print("Анализ активности ОП...")

    # 1. Распределение активности ОП
    plt.figure(figsize=(12, 6))
    op_activity_counts = df['op_activity'].value_counts()
    colors = ['red', 'orange', 'yellow', 'green']
    sns.barplot(x=op_activity_counts.index, y=op_activity_counts.values, palette=colors)
    plt.title('Распределение тредов по активности ОП', fontsize=16)
    plt.xlabel('Уровень активности ОП', fontsize=14)
    plt.ylabel('Количество тредов', fontsize=14)
    plt.grid(True, axis='y')
    plt.tight_layout()
    plt.savefig('stats/op_activity_distribution.png', dpi=300)
    plt.close()

    # 2. Зависимость активности ОП от длины его поста
    plt.figure(figsize=(14, 6))
    sns.boxplot(x='op_activity', y='op_comment_length', data=df, palette='Set2')
    plt.title('Зависимость активности ОП от длины его изначального поста', fontsize=16)
    plt.xlabel('Активность ОП', fontsize=14)
    plt.ylabel('Длина комментария ОП (символы)', fontsize=14)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('stats/op_activity_vs_comment_length.png', dpi=300)
    plt.close()

    # 3. Корреляция между активностью ОП и популярностью треда
    plt.figure(figsize=(14, 6))
    sns.scatterplot(x='op_reply_ratio', y='posts_count', hue='has_op_files',
                    size='unique_posters', sizes=(20, 200), alpha=0.7, data=df)
    plt.title('Связь между активностью ОП и популярностью треда', fontsize=16)
    plt.xlabel('Доля ответов ОП', fontsize=14)
    plt.ylabel('Количество постов в треде', fontsize=14)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('stats/op_replies_vs_popularity.png', dpi=300)
    plt.close()

def analyze_file_content(df):
    """Анализ файлового контента"""
    print("Анализ файлового контента...")

    # 1. Распределение тредов с файлами и без
    plt.figure(figsize=(10, 6))
    files_counts = df['has_files'].value_counts()

    # Создаем правильные лейблы и цвета в соответствии с имеющимися данными
    labels = []
    colors = []
    for value in files_counts.index:
        if value:
            labels.append('С файлами')
            colors.append('lightblue')
        else:
            labels.append('Без файлов')
            colors.append('lightcoral')

    plt.pie(files_counts.values, labels=labels, colors=colors, autopct='%1.1f%%', startangle=90)
    plt.title('Распределение тредов по наличию файлов', fontsize=16)
    plt.tight_layout()
    plt.savefig('stats/files_distribution.png', dpi=300)
    plt.close()

    # 2. Среднее количество файлов на пост
    plt.figure(figsize=(12, 6))
    sns.histplot(df['files_per_post'], bins=20, kde=True, color='green')
    plt.title('Распределение количества файлов на пост', fontsize=16)
    plt.xlabel('Файлов на пост', fontsize=14)
    plt.ylabel('Количество тредов', fontsize=14)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('stats/files_per_post.png', dpi=300)
    plt.close()

    # 3. Влияние файлов в ОП на популярность треда
    plt.figure(figsize=(12, 6))
    sns.boxplot(x='has_op_files', y='posts_count', data=df, palette='Set3')
    plt.title('Влияние наличия файлов в ОП на популярность треда', fontsize=16)
    plt.xlabel('Наличие файлов в ОП посте', fontsize=14)
    plt.ylabel('Количество постов в треде', fontsize=14)
    plt.xticks([0, 1], ['Нет файлов в ОП', 'Есть файлы в ОП'])
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('stats/op_files_vs_popularity.png', dpi=300)
    plt.close()

def analyze_thread_titles(df):
    """Анализ заголовков тредов"""
    print("Анализ заголовков тредов...")

    # 1. Распределение длины заголовков
    plt.figure(figsize=(14, 6))
    sns.histplot(df['title_len'], bins=30, kde=True, color='purple')
    plt.title('Распределение длины заголовков тредов', fontsize=16)
    plt.xlabel('Длина заголовка (символы)', fontsize=14)
    plt.ylabel('Количество тредов', fontsize=14)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('stats/title_length_distribution.png', dpi=300)
    plt.close()

    # 2. Категории заголовков
    plt.figure(figsize=(12, 6))
    title_category_counts = df['title_category'].value_counts()
    sns.barplot(x=title_category_counts.index, y=title_category_counts.values, palette='plasma')
    plt.title('Распределение тредов по категориям длины заголовков', fontsize=16)
    plt.xlabel('Категория длины заголовка', fontsize=14)
    plt.ylabel('Количество тредов', fontsize=14)
    plt.grid(True, axis='y')
    plt.tight_layout()
    plt.savefig('stats/title_categories.png', dpi=300)
    plt.close()

    # 3. Влияние длины заголовка на популярность
    plt.figure(figsize=(14, 6))
    sns.scatterplot(x='title_len', y='posts_count', hue='title_category',
                    size='unique_posters', sizes=(20, 200), alpha=0.7, data=df)
    plt.title('Влияние длины заголовка на популярность треда', fontsize=16)
    plt.xlabel('Длина заголовка (символы)', fontsize=14)
    plt.ylabel('Количество постов', fontsize=14)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('stats/title_length_vs_popularity.png', dpi=300)
    plt.close()

def analyze_thread_duration(df):
    """Анализ длительности жизни тредов"""
    print("Анализ длительности жизни тредов...")

    # Фильтруем треды с валидной длительностью
    valid_duration_df = df[df['thread_duration_hours'] > 0]

    if len(valid_duration_df) == 0:
        print("Нет данных о длительности тредов")
        return

    # 1. Распределение длительности жизни
    plt.figure(figsize=(14, 6))
    sns.histplot(valid_duration_df['thread_duration_hours'], bins=30, kde=True, color='orange')
    plt.title('Распределение длительности жизни тредов', fontsize=16)
    plt.xlabel('Длительность (часы)', fontsize=14)
    plt.ylabel('Количество тредов', fontsize=14)
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('stats/thread_duration_distribution.png', dpi=300)
    plt.close()

    # 2. Категории длительности
    plt.figure(figsize=(12, 6))
    duration_category_counts = valid_duration_df['duration_category'].value_counts()
    sns.barplot(x=duration_category_counts.index, y=duration_category_counts.values, palette='coolwarm')
    plt.title('Распределение тредов по категориям длительности жизни', fontsize=16)
    plt.xlabel('Категория длительности', fontsize=14)
    plt.ylabel('Количество тредов', fontsize=14)
    plt.xticks(rotation=45)
    plt.grid(True, axis='y')
    plt.tight_layout()
    plt.savefig('stats/duration_categories.png', dpi=300)
    plt.close()
...
def main():
    ...
    try:
        ...
        analyze_thread_activity(df)
        analyze_op_activity(df)
        analyze_file_content(df)
        analyze_thread_titles(df)
        analyze_thread_duration(df)
